...
class SacyRoomCustomersController < ApplicationController
  ...
  def index
    if params[:search].present?
      @sacy_room_customers = SacyRoomCustomer.sacy_room_search(params[:search]).paginate(page: params[:page], per_page: 150)
      respond_to do |format|
        format.html
        format.csv { send_data @sacy_room_customers.to_csv(['name', 'email', 'genre', 'city', 'country'])}
      end
    else
      @sacy_room_customers = SacyRoomCustomer.all.paginate(page: params[:page], per_page: 150)
      respond_to do |format|
        format.html
        format.csv { send_data @sacy_room_customers.to_csv(['name', 'email', 'genre', 'city', 'country'])}
      end
