...
class RabaSpaCustomersController < ApplicationController
  ...
  def index
    if params[:search].present?
      @raba_spa_customers = RabaSpaCustomer.raba_spa_search(params[:search]).paginate(page: params[:page], per_page: 150)
      respond_to do |format|
        format.html
        format.csv { send_data @raba_spa_customers.to_csv(['name', 'email', 'genre', 'city', 'country'])}
      end
    else
      @raba_spa_customers = RabaSpaCustomer.all.paginate(page: params[:page], per_page: 150)
      respond_to do |format|
        format.html
        format.csv { send_data @raba_spa_customers.to_csv(['name', 'email', 'genre', 'city', 'country'])}
      end
