...
class LeognanCustomersController < ApplicationController
  ...
  def index
    if params[:search].present?
      @leognan_customers = LeognanCustomer.leognan_search(params[:search]).paginate(page: params[:page], per_page: 150)
      respond_to do |format|
        format.html
        format.csv { send_data @leognan_customers.to_csv(['name', 'email', 'genre', 'city', 'country'])}
      end
    else
      @leognan_customers = LeognanCustomer.all.paginate(page: params[:page], per_page: 150)
      respond_to do |format|
        format.html
        format.csv { send_data @leognan_customers.to_csv(['name', 'email', 'genre', 'city', 'country'])}
      end
