...
class TheouleCustomersController < ApplicationController
  ...
  def index
    if params[:search].present?
      @theoule_customers = TheouleCustomer.theoule_search(params[:search]).paginate(page: params[:page], per_page: 30)
      respond_to do |format|
        format.html
        format.csv { send_data @theoule_customers.to_csv(['name', 'email', 'genre', 'city', 'country'])}
      end
    else
      @theoule_customers = TheouleCustomer.all.paginate(page: params[:page], per_page: 30)
      respond_to do |format|
        format.html
        format.csv { send_data @theoule_customers.to_csv(['name', 'email', 'genre', 'city', 'country'])}
      end
