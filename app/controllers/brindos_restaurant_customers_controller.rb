...
class BrindosRestaurantCustomersController < ApplicationController
  ...
  def index
    if params[:search].present?
      @brindos_restaurant_customers = BrindosRestaurantCustomer.brindos_restaurant_search(params[:search]).paginate(page: params[:page], per_page: 30)
      respond_to do |format|
        format.html
        format.csv { send_data @brindos_restaurant_customers.to_csv(['name', 'email', 'genre', 'city', 'country'])}
      end
    else
      @brindos_restaurant_customers = BrindosRestaurantCustomer.all.paginate(page: params[:page], per_page: 30)
      respond_to do |format|
        format.html
        format.csv { send_data @brindos_restaurant_customers.to_csv(['name', 'email', 'genre', 'city', 'country'])}
      end
