...
class MegeveRoomCustomersController < ApplicationController
  ...
  def index
    if params[:search].present?
      @megeve_room_customers = MegeveRoomCustomer.megeve_room_search(params[:search]).paginate(page: params[:page], per_page: 150)
      respond_to do |format|
        format.html
        format.csv { send_data @megeve_room_customers.to_csv(['name', 'email', 'genre', 'city', 'country'])}
      end
    else
      @megeve_room_customers = MegeveRoomCustomer.all.paginate(page: params[:page], per_page: 150)
      respond_to do |format|
        format.html
        format.csv { send_data @megeve_room_customers.to_csv(['name', 'email', 'genre', 'city', 'country'])}
      end
