class MegeveRestaurantCustomer < ApplicationRecord
  validates :email, format: { with:  /\A[^@\s]+@[^@\s]+\z/}, allow_blank: true
  validates :email, uniqueness: true
  include PgSearch::Model
  pg_search_scope :megeve_restaurant_search,
  ...
  def self.import(file)
    CSV.foreach(file.path, headers: true) do |row|
      MegeveRestaurantCustomer.where(email: row[3]).first_or_create do |c|
        c.genre = row[0]
        c.name = row[1]
        c.surname = row[2]
        c.country = row[4]
